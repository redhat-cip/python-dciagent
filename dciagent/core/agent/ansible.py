...
# under the License.
"""
Module for the base ansible agent.
"""
...
import dciagent.core.agent.base as base
import dciagent.core.error as error


class Agent(base.Agent):
    """
    Main ansible agent class.

    All ansible agents should inherit from here. A vanilla ansible agent as
    defined in here should run with a default ansible configuration, similar to
    how a single `ansible-playbook` call would work in linux.
    """
    ...
    def _validate(self):
        ...
        if playbook is None:
            raise (error.ValidationError("Please provide a path to a playbook."))
        ...
        if cfg is None:
            raise (
                error.ValidationError("Please provide a valid path to the ansible.cfg")
            )
        else:
            if not os.path.isfile(cfg):
                raise (
                    error.ValidationError("Cannot read ansible config {}".format(cfg))
                )
